...
% notes: (agentic web is trending → CUI is weak → we need generative UI → there is hallucination → needed grounding → frameworks different but no grounding → hypermedia is a good solution → affordances are the right abstraction → this thesis is about that)
...
\subsubsection{Rendering-side vs. Source-side}
...
\subsubsection{Rendering Frameworks}
...
\subsubsection{Action Grounding Approaches}
...
\subsection{Affordance-Driven Human--Agent Interaction}\label{sec:bg:hai}

Problems that generative UI have ...

To align the action space that the agent offers with what the human user expects. They sharing a same wordlist of valid actions, which is affordance. Namely what it may offer, ask, and refuse — offers are always executable, refusals are structural/explainable, required form-fields define exactly what to ask.

\paragraph{Limitations of this thesis}
...
\subsection{Research Gap}\label{sec:bg:gap}
